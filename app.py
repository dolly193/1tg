...
def finalize_file_processing(filename, user_id, conn, magnet_link=None):
    ...
    # Lê o conteúdo binário do arquivo para salvar no banco
    with open(file_path, 'rb') as f:
        file_blob = f.read()
    ...
    cursor.execute(sql, (file_hash, filename, file_size, user_id, magnet_link, file_blob))
...
def download_content(filename):
    ...
    if conn:
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT file_content FROM arquivos_dolly WHERE filename = %s", (filename,))
        row = cursor.fetchone()
        conn.close()
